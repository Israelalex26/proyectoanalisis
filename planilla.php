...
$sql = "SELECT id_planilla, nombre_empleado , dpi , afiliacion,departamento,sueldo_base,igss,igss_patrono,fecha FROM planilla WHERE fecha LIKE '%$search%'";
...
<body>
    ...
    <form action="" method="GET" class="my-3">
        <div class="input-group">
            <input type="date" class="form-control" placeholder="Buscar de liquidacion" name="search" aria-label="Buscar" aria-describedby="btnBuscar">
            <button class="btn btn-outline-secondary" type="submit" id="btnBuscar">Buscar</button>
        </div>
    </form>

    <!-- Botón para agregar departamento con margen -->
<form action="planilla/planilla.php" method="GET" style="margin-left: 30px;">
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>" . $row["igss"] . "</td>";
                 echo "<td>" . $row["igss_patrono"] . "</td>";
                echo "<td>" . $row["fecha"] . "</td>";
                echo "<td>
                        <a href='pdf/planilla.php?id=" . $row["id_planilla"] . "' target='_blank' class='btn btn-primary'>PDF</a>
                      </td>";
