...
if ($conn) {
    ...
    // Consulta SQL para obtener los datos de la tabla "historialsalario" con filtro por código de empleado o salario anterior
    $sql = "SELECT id_historialsalario, salario_anterior, salario_nuevo, fk_cod_empleado FROM historialsalario WHERE fk_cod_empleado LIKE '%$search%'";
...
<body>
 ...
 <header>
    ...
    <table>
        ...
        <?php
        // Mostrar los datos obtenidos de la tabla "historialsalario"
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["fk_cod_empleado"] . "</td>";
                echo "<td>" . obtenerNombreEmpleado($row["fk_cod_empleado"], $conn) . "</td>";
                echo "<td>" . $row["salario_anterior"] . "</td>";
                echo "<td>" . $row["salario_nuevo"] . "</td>";
                echo "<td>
                        <button class='btn editar-btn' onclick='editarEmpleado(\"" . $row["id_historialsalario"] . "\")'>Editar</button>
                      </td>";
                echo "</tr>";
            }
        } else {
    ...
    <script>
        function editarEmpleado(codEmpleado) {
            window.location.href = "http://nominasolidarista.wuaze.com/historialsalario/editar_historialsalario.php?id=" + codEmpleado;
        }
    </script>
