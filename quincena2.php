...
$sql = "SELECT id_quincena2, empleado ,salario, resto,comisiones,igss,irtra,isr,Total,mes,ano FROM quincena2 WHERE empleado LIKE '%$search%'";
 ...
 <header>
    ...
    <form action="" method="GET" class="my-3">
        <div class="input-group">
            <input type="text" class="form-control" placeholder="Buscar por el nombre del empleado" name="search" aria-label="Buscar" aria-describedby="btnBuscar">
            <button class="btn btn-outline-secondary" type="submit" id="btnBuscar">Buscar</button>
        </div>
    </form>

    <!-- Botón para agregar departamento con margen -->
<form action="quincena2/quincena2.php" method="GET" style="margin-left: 30px;">
    <button class="btn btn-outline-secondary" type="submit" style="background-color: #D99C53; color: black;">
        &#9733; Agregar segunda quincena
    </button>
</form>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>
               <a href='pdf/quincena2.php?id=" . $row["id_quincena2"] . "' target='_blank' class='btn btn-primary'>PDF</a>
                </td>";
                echo "</tr>";
            }
        } else {
